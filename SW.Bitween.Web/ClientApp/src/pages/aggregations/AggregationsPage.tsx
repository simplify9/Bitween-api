...
export function AggregationsPage() {
  ...
  const rows = useQuery({
    queryKey: ["integration-rows-search", "Aggregation", q, offset],
    queryFn: () => api.searchIntegrationRows({ search: q, type: "Aggregation", offset, limit: PAGE_SIZE }),
  ...
  return (
    <div>
      ...
      {rows.isPending ? (
        ...
        <EmptyState icon={<Layers />} title={q ? "No aggregations match" : "No aggregations yet"}>
          {q
