  ...
  return (
    ...
    >
      <div className="flex min-w-max items-stretch justify-center">
        {faces.map((face, i) => {
          const isSelected = selected === face.id;
          const Icon = STAGES[face.id].icon;
          ...
          return (
            <Fragment key={face.id}>
              ...
              <button
                ...
                title={isSelected ? "Click again to close and show the overview" : undefined}
                className={`w-58 shrink-0 cursor-pointer rounded-xl border px-4 py-3.5 text-left transition-shadow ${border} ${
              ...
              >
                ...
                <p
                  className={`mt-2.5 truncate text-[15px] font-semibold ${titleTone[face.state]}`}
                  title={face.title}
                >
                  {face.title}
                </p>
                {/*
                  Always rendered so every node keeps the same baseline — and
                  with a non-breaking space, not a plain one: `truncate` sets
                  `white-space: nowrap`, which collapses an ordinary space away
                  to nothing. The <p> then had no line box, and since a <button>
                  centres its content, every node without a detail line sat 9px
